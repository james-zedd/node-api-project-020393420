...
exports.addCourse = asyncHandler(async (req, res, next) => {
  ...
  // Authenticate user to add course (must own bootcamp)
  if (bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin') {
    return next(new ErrorResponse(`User with user id ${req.user.id} is not authorized to add a course to this bootcamp ${bootcamp._id}`, 401));
  } 
...
exports.updateCourse = asyncHandler(async (req, res, next) => {
  ...
  // Authenticate user to update course (must own course)
...
exports.deleteCourse = asyncHandler(async (req, res, next) => {
  ...
  // Authenticate user to delete course (must own course)
  if (course.user.toString() !== req.user.id && req.user.role !== 'admin') {
